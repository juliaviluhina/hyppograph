...
- [ ] T001 Create `tests/harness/` layout per plan.md (`service.mjs`, `run.mjs`,
...
- [ ] T002 [P] Add `npm run harness` script to `package.json` invoking `tests/harness/run.mjs`;
      confirm `npm test` (`node --test evals/component/`) is unaffected
- [ ] T003 [P] Write `tests/harness/support/ports.mjs`: fixed-port bind with fail-fast on
...
- [ ] T004 Implement `tests/harness/service.mjs` per `contracts/fixture-service.md`: ATS-shaped
...
- [ ] T005 Implement `tests/harness/support/scratch.mjs`: copy `tests/fixtures/data-dir/` to a
      temp dir per run, return paths; runner deletes on success, keeps on failure (with the path
      printed) for debugging
- [ ] T006 Implement `tests/harness/support/isolation.mjs`: two-sided FR-005 assertion — every
...
- [ ] T007 Add the `ATS_API_BASE_OVERRIDES` host→origin map read at the `buildAtsApiUrl` site in
...
- [ ] T008 Implement `tests/harness/run.mjs` skeleton: service start → scratch copy → case
...
- [ ] T009 [P] Extend `tests/fixtures/data-dir/` where the expected matrix needs it (ATS-shaped
...
- [ ] T010 [US1] Implement the full-flow case in `tests/harness/regression-cases/full-flow.mjs`:
      scratch dir + override map → run verify→score → assert per-record verdicts/flags/outcomes
      against `contracts/expected-matrix.md`
- [ ] T011 [US1] Assert the run summary counts (verified open/closed/unresolvable, scored,
...
- [ ] T012 [US1] Wire the FR-005 isolation proof into the full-flow case (service access log
...
- [ ] T014 [P] [US2] Verify case in `tests/harness/worker-cases/verify-signal.mjs`: live →
...
- [ ] T015 [P] [US2] Score case in `tests/harness/worker-cases/score-cited-rows.mjs`: gap fixture
...
- [ ] T016 [P] [US2] Settings-reader case in `tests/harness/worker-cases/settings-verbatim.mjs`:
      tricky paths echoed byte-for-byte (runs 1/4/6 class)
- [ ] T017 [P] [US2] Evidence-reader case in `tests/harness/worker-cases/evidence-single-read.mjs`:
      one call per file, each result echoes its own path+content (run 5 class)
- [ ] T018 [P] [US2] Applications-reader case in
...
- [ ] T019 [P] [US2] Citation-audit case in `tests/harness/worker-cases/citation-advisory.mjs`:
...
- [ ] T022 [US3] Service-stopped case: with the service down, dependent cases fail fast with
      "fixture service unreachable" and no live-host fallback is attempted (assert via isolation
      proof)
- [ ] T023 [US3] Closed/malformed matrix wired to the service scenarios from
...
- [ ] T024 [P] [US4] Config-gate case in `tests/harness/regression-cases/config-gate.mjs`:
      stripped-prefix / empty-evidence / missing-recency-window fixtures →
      `config.evidence-unavailable`, zero writes (run 1)
- [ ] T025 [P] [US4] Worker-presence case in
...
- [ ] T026 [P] [US4] Fixture-honesty case in
...
- [ ] T027 [US4] Idempotency case in `tests/harness/regression-cases/idempotency.mjs`:
...
- [ ] T028 [US4] Fault-injection sweep: for each case above, reintroduce its historical fault and
...
- [ ] T029 [P] Harness docs: one short section (service lifecycle, `--case=` debug flag,
...
- [ ] T031 Confirm `npm test` unaffected, `package.json` still has zero dependencies, and no
      committed fixture was mutated in place by any harness run (`git status` clean on
      `tests/fixtures/` after a full cycle)
- [ ] T032 Record the 005 exit state for 006 intake: current reds (expected: exactly T024),
