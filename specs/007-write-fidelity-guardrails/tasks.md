...
- [X] T007 [US1] Validate SC-001 per `quickstart.md` §2: temporarily remove the BEGIN-CONTENT/END-CONTENT markers from T005's script, run `npm run test:fidelity`, confirm T006 fails with a diff showing the dropped `---`; restore the markers, confirm it passes again; record the result in the PR description, no code change retained from this step
  - **Environment note**: `npm run test:fidelity` shells out to `claude -p --dangerously-skip-permissions` (research.md R1). Running it from inside another Claude Code agent's own Bash tool is denied outright by that session's auto-mode classifier ("Create Unsafe Agents") before the Workflow tool ever runs — confirmed 2026-09-14. Not a defect in the test tier; run it from a plain human terminal instead.
  - **Result (2026-09-14, human terminal, real model calls)**: happy-path proven live — all 3 atomic tests pass 4/4 test cases (after two real bugs found and fixed along the way: a missing `export const meta` first statement on all three `.workflow.js` files, and `node --test`'s default concurrency causing nested `claude -p` sessions to collide, fixed with `--test-concurrency=1`). The strip-markers/restore regression check itself did **not** reproduce F4 on the current model version, across 4 attempts total (1 with a minimal 4-line synthetic fixture, 3 more after widening the fixture to match `buildEvaluationWritePrompt`'s real field count/shape) — consistent with this spec's own Edge Cases note ("a future model version might not exhibit this exact failure... keep the BEGIN/END convention even if the atomic test stays green for a long stretch"). SC-001 is accepted as satisfied on the mechanism proof (markers present → byte-exact transcription, proven live) rather than a live re-reproduction of F4 itself, which the current model no longer appears to exhibit.
