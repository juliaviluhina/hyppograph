...
- [ ] T001 Create `.claude/workflows/fit-screen.js` skeleton: `export const meta` first
      ...
      `intake-normalize.js`
- [ ] T002 [P] Extend `tests/fixtures/data-dir/inputs/settings.json` (and
...
- [ ] T003 [P] Create `tests/fixtures/data-dir/inputs/evidence/career-history.md` and
      ...
      Records in T004
- [ ] T004 [P] Create/extend fixture Job Records under `tests/fixtures/data-dir/outputs/job-records/`
...
- [ ] T005 [P] Extend `tests/fixtures/data-dir/inputs/applications.md` with rows covering: a
...
- [ ] T006 [P] Create `tests/fixtures/live/job-records/` with one real, currently-open
...
- [ ] T007 In `.claude/workflows/fit-screen.js`, define the `agent()` JSON schemas as top-level
...
- [ ] T008 [P] Write `specs/004-retrieval-fit-screen-rework/contracts/schemas.md` documenting those
      two schemas and which `agent()` call uses each (mirrors feature 001's `contracts/schemas.md`)
- [ ] T009 In the script, implement inline helpers needed by this feature (duplicated from, and kept
...
- [ ] T010 In the script body: load `${args.dataDir}/inputs/settings.json`; run this feature's own
      ...
      top-level `return` it.
- [ ] T011 Apply the shared subagent policy (mirrors feature 001's T011) to every new `agent()` call
      ...
      `model: "<mid-tier alias>"` — the one non-fast call in this feature (Principle II); (e) every
...
- [ ] T012 [P] Create `.claude/agents/hyppo-verify.md`: `tools: WebFetch`, fast tier, told to return
...
- [ ] T013 [P] Create `.claude/agents/hyppo-score.md`: `tools: Read, Glob`, mid tier, told to read
      only the exact evidence-file paths named in the prompt, cite every non-`Unknown` verdict with
      an exact file+section, and never assign the final `overallVerdict` (the script recomputes it
...
- [ ] T014 [US1] Read `sections.evidenceBase.value.files` via `hyppo-read`; resolve each path under
      `args.dataDir` and fail the run-start gate (T010) if any is empty/unreadable
      (`contracts/settings-additions.md`)
- [ ] T015 [US1] Read `sections.hardConstraints.value` (`compFloor`, `excludedRoleNatures`) via
...
- [ ] T016 [US1] Read `sections.targetRoles.value` (`streams[]`, `recencyWindowYears`) via
      `hyppo-read` (FR-005's seniority-band and recency-discount inputs)
- [ ] T017 [US1] Add the `score` phase: `pipeline(scorableRecords, async (rec) => …)` (no options
...
- [ ] T018 [US1] Per record, one `hyppo-score` `agent()` call (mid tier, `fitEvaluationSchema`) with
...
- [ ] T019 [US1] Hard-constraint section: from the same call's output, record each of `compFloor`,
      ...
      `"unknown"` maps to `unresolved`, never assumed pass (FR-004a)
- [ ] T020 [US1] Anti-pattern flags: from the same call's output, record domain-crossover overclaim,
...
- [ ] T021 [US1] Compute the final `overallVerdict` **in the script**, not trusted from `hyppo-score`
      ...
      hard constraint `fail` ⇒ `SKIP` overriding the above; an `unresolved` hard constraint caps at
      `APPLY-AND-SEE` (or forces `SKIP` if more likely than not to fail); otherwise `APPLY` (FR-006,
      `contracts/evaluation-format.md`)
- [ ] T022 [US1] `citation_audit` delegation: when a run batch persists 2+ evaluations, one
      ...
      malformed audit result blocks persistence of that evaluation the same run (FR-010/FR-011,
      logged per T023)
- [ ] T023 [US1] Persist via `hyppo-readwrite`: write `outputs/evaluations/<key>.md` per
      `contracts/evaluation-format.md` (front matter + rendered body), append the delegation log
      entry (T022) inline, append a provenance line (FR-012, FR-015)
...
- [ ] T026 [US2] `buildAtsApiUrl(sourceRef)` (T009): recognize Greenhouse/Lever/Ashby posting-host
      ...
      (research.md R5)
- [ ] T027 [US2] Add the `verify` phase **before** `score`: a **serial `for` loop** (like feature
...
- [ ] T028 [US2] For each record where `buildAtsApiUrl` returned a URL, one `hyppo-verify` `agent()`
      call (fast tier, `verifySignalSchema`) against that URL, returning only the raw signal
      (research.md R6)
- [ ] T029 [US2] `mapSignalToMark(signal)` (T009) in code: `found`→`confirmed-open`,
      ...
      no signal available"`
- [ ] T030 [US2] Pacing: reuse feature 001's `HYPPO_PACING_MS`/`HYPPO_FETCH_CAP` args/config for this
      loop's re-check cadence — no second pacing mechanism (FR-002d)
- [ ] T031 [US2] Write the mark via `hyppo-readwrite` (`openStatus`, `openStatusCheckedAt`,
      `openStatusReason`) **only when it changed** from the previous run — no duplicate write or
      provenance line when a re-check reproduces the same mark (FR-002c/FR-009); narrow the `score`
      phase's `scorableRecords` (T017) to `confirmed-open`/`unresolvable` records only —
      `confirmed-closed` is excluded and its exclusion recorded (FR-002a/b)
- [ ] T032 [US2] Populate `RunSummary.verifiedConfirmedOpen/Closed/Unresolvable`; append a
      provenance line only for a newly-set or changed mark (FR-015, FR-020-equivalent)
...
- [ ] T034 [US3] Read `inputs/applications.md` via `hyppo-read`, reusing feature 001's tolerant
      parsing; distinguish "file missing entirely" from "file present with zero/no matching rows"
      (research.md R3)
- [ ] T035 [US3] Application-state reconciliation (folded into the `hyppo-score` call's scope, T018):
...
- [ ] T036 [US3] Migrate the Job Record on write (via `hyppo-readwrite`): replace the old
...
- [ ] T037 [US3] Mirror `applicationState` onto the FitEvaluation front matter at scoring time
      (`contracts/evaluation-format.md`)
...
- [ ] T039 [US4] Confirm T010's run-start gate reports `config.evidence-unavailable` (not free text)
...
- [ ] T040 [US4] Tag `open.unresolved` on any Job Record's evaluation whose `openStatus` is
...
- [ ] T041 [US4] Tag `score.insufficient-input` for a kept, verified Job Record too sparse to build a
...
- [ ] T042 [US4] Tag `state.ambiguous-match` alongside the `ambiguous` `ApplicationStateValue` from
      T035 (FR-008's fourth vocabulary entry)
- [ ] T043 [US4] Populate `RunSummary.namedOutcomeCounts`; review every non-clean code path in the
...
- [ ] T045 [P] Run-summary rendering: write/extend `${args.dataDir}/outputs/last-run-summary.md`
      with every field from data-model.md's `RunSummary`, and `log()` a human-readable rendering
- [ ] T046 [P] Create the committed, fabricated-persona settings + evidence template (FR-000a) —
...
- [ ] T047 Constitution audit — Principles I, II, IV: confirm `fit-screen.js`'s two `phase()` markers
...
- [ ] T048 Constitution audit — Principle V: confirm every write in this feature is under
...
- [ ] T051 [P] Save the workflow as a project command via `/workflows` → `s`; update
      `quickstart.md` Phase A if the invocation name differs from `/fit-screen`
- [ ] T052 [P] Add a docs note (mirroring feature 001's `docs/pipeline.md`) that verify→score is live
