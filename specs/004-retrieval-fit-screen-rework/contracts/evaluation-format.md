...
requirementTable:
  - requirement: "5+ years building internal platforms"
    verdict: "Strong"
    evidenceFile: "evidence/career-history.md"
...
evidenceFilesUsed: ["evidence/career-history.md", "evidence/cv-content.md"]
---
...
|---|---|---|
| 5+ years building internal platforms | Strong | evidence/career-history.md § Acme-adjacent platform work, 2021-2024 |
| Experience leading a platform team | Partial | evidence/career-history.md § Tech lead, 3-person team, 2023 — adjacent, not the thing asked for |
...
- `hardConstraints[].state` ∈ `pass` / `fail` / `unresolved` — a closed, separate scale from
  `requirementTable`'s (FR-004, data-model.md Verdict Scale entity).
- `overallVerdict` MUST be derivable from `requirementTable` + `hardConstraints` alone by the FR-006
  ...
  constraint `fail` ⇒ `SKIP`; an `unresolved` hard constraint caps at `APPLY-AND-SEE` or forces
  `SKIP`) — a citation_audit or later manual read MUST be able to recompute it from the table, never
  trust a verdict the table doesn't support (SC-003).
