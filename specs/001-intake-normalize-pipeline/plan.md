...
| Tests | Tier 1 pure-code unit (`node:test`) + Tier 3 golden synthetic integration run + manual quickstart — see [eval-strategy.md](./eval-strategy.md) | Tier 1/2/3 ported onto B2's stack: `vitest` unit + integration, SC-006 re-run assertion. **CI deferred indefinitely** — no runs outside the user's control (eval-strategy.md §6.3); metered runs are manual with a spend guard |
