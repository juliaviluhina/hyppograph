...
|---|---|---|---|
| **I. Deterministic, Code-Driven Orchestration** | The workflow **script** sequences `phase()` → `pipeline()` → `agent()`; no model decides what runs next. Each `agent()` is a bounded subtask. *Looser point, accepted for a prototype:* a subagent runs multiple turns and has some latitude within its task, vs. a `maxTurns: 1` call. | `run.ts` is plain control flow; every judgment is `query({ maxTurns: 1 })` with no tools. No agent loop. | PASS (A: PASS-with-note) |
| **II. Right-Tier Model Usage** | Every `agent()` that makes a judgment sets `model: "haiku"` (fast tier) explicitly. | `judge({ tier })` wrapper; `config` maps `fast → haiku`. Single audit point. | PASS |
