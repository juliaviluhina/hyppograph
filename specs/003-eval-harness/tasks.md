...
- [X] T002 [P] Edit `.env.example` — add the judge credential placeholder name (name only, no value);
  leave a commented placeholder for the future metered-substrate credential (FR-017). Addendum
  (2026-09-14, post-hoc): `HYPPO_JUDGE_EFFORT` and the `x-opencode-session` header requirement were
  discovered live and added after this task was first completed — see `research.md` D5 and FR-010b.
...
- [X] T004 Create `.claude/workflows/lib/intake-core.mjs` — move the pure helpers out of
...
- [X] T005 Create `.claude/workflows/lib/prompts.mjs` — move the `agent()` prompt text out of
...
- [X] T006 Edit `.claude/workflows/intake-normalize.js` — replace the helper bodies with a
  ...
  Verified 2026-09-14: the workflow file already carried this exact inlined copy from its original
  authoring; `intake-core.mjs` was written to match it byte-for-byte, confirmed by a stripped-module
  vs. marker-region diff — no edit to `intake-normalize.js` was needed.
- [X] T007 Edit `.claude/workflows/intake-normalize.js` — replace the inline `agent()` prompt text
  with a marker-delimited `inlined:prompts` region generated the same way from `prompts.mjs`. Same
  verification as T006: already byte-identical, no edit needed.
- [X] T008 [P] Create `evals/lib/mock-agent.mjs` — deterministic fixture-canned `agent()` for
  plumbing debug: given a case id, return a pre-recorded structured result; zero cost, no network,
  ignores pacing (research D7).
- [X] T009 Create `evals/run.mjs` skeleton — parse `<layer>` + options per `contracts/evals-cli.md`
...
- [X] T010 [P] [US1] `evals/component/keys.test.mjs` — `node:test` cases for `titleKey`, `companyKey`,
  ...
  regressions (SC-002).
- [X] T011 [P] [US1] `evals/component/folding.test.mjs` — `foldSet` / set-folding, `renderSummary`,
  `newRunSummary`, `bumpReason`, `provenanceLine`.
- [X] T012 [P] [US1] `evals/component/criteria.test.mjs` — `criteriaFingerprint`, `noTriageCriteria`,
  `effectiveExcludedLocations`, `completenessLevel` (the low-completeness threshold rule).
- [X] T013 [P] [US1] `evals/component/sources.test.mjs` — `trackedBoardsToSources` mapping.
- [X] T014 [US1] `evals/component/drift.test.mjs` — for each region (`intake-core`, `prompts`): read
...
- [X] T015 [US1] Wire the `component` layer into `evals/run.mjs` (delegate to `node --test`, map exit
  0/1) and confirm `npm test` resolves to it (contract: `contracts/evals-cli.md`).
- [X] T016 [US1] Run the Independent Test above; confirm whole suite < 5 s, zero network, zero model
