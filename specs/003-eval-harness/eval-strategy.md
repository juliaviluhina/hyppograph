# Eval Strategy: Intake & Normalize Pipeline — companion design notes

**Feature**: 003-eval-harness (design precursor) | **Status**: superseded by [spec.md](./spec.md) + [plan.md](./plan.md) | **Date**: 2026-09-02

Moved here from `specs/001-intake-normalize-pipeline/eval-strategy.md` per FR-021 (pointer left at the
old location). This is the worked-out reasoning behind feature 003 — the three-tier pyramid, the
non-Claude judge, spend controls, substrate split, evidence reports. Where it and the spec/plan
differ in wording, **the spec and plan are authoritative**; this file is kept for the rationale and
the bug history in §1. It also elaborates the **Tests** row of feature 001's
[plan.md § Phasing](../001-intake-normalize-pipeline/plan.md).
