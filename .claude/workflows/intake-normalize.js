 ...
 *
 * RUNTIME CONTRACT (Claude Code dynamic workflow sandbox — https://code.claude.com/docs/en/workflows)
 *   - Constrained JavaScript: no `import`, no `require`, no direct fs / shell / network from the
 *     workflow body. `Date.now()` and `Math.random()` THROW — every timestamp/id comes from `args`.
 *   - Orchestration primitives provided to the body (as globals):
 *       phase(name, fn)                     — declare + run one ordered stage
 *       pipeline(items, fn, { concurrency })— map fn over items (<= 4096 items, <= 16 concurrent)
 *       parallel(...fns)                    — run a fixed set of thunks together
 *       agent(prompt, { schema, label, model, allowedTools }) — one bounded subtask; returns an
 *                                             object matching `schema` (and nothing else)
 *       log(...)                            — structured progress log, surfaced in /workflows
 *   - `args` (passed to the default export):
 ...
 *       args.pacingMs?    : number  — default 3000  (HYPPO_PACING_MS)
 ...
 *   IV. Collect agents get read-only HyppoVisor tools + Write; triage/normalize agents get Read + Write.
 *       No agent is ever given Edit, Bash, or any submit/send/apply capability.
...
export const meta = {
  ...
  description:
    "Collect job postings from tracked board searches via HyppoVisor, store them verbatim with " +
    "provenance, pre-triage each against hard stops + considered directions, and normalize kept " +
    "postings into comparable Job Records with cross-source dedup. All state is plain files under " +
    "HYPPO_DATA_DIR. Reads inputs/settings.json (feature 002). No outward-facing actions.",
  phases: ["collect", "triage", "normalize"],
};

/* ------------------------------------------------------------------ *
 * Subagent tool policy (T011 clause a) — Principle IV
 * ------------------------------------------------------------------ */
const HV = "mcp__hyppovisor__"; // read/navigation tools only — see .mcp.json + contracts/hyppovisor-page-read.md
const HV_READ_TOOLS = [
  HV + "open_url",
  HV + "navigate",
  HV + "wait_for_selector",
  HV + "read_page",
  HV + "read_form_fields",
  HV + "list_open_tabs",
];
const COLLECT_TOOLS = [...HV_READ_TOOLS, "Write"]; // fetch pages + write Raw Records
const READ_WRITE_TOOLS = ["Read", "Write"]; // triage + normalize touch only local files
...
/* ------------------------------------------------------------------ *
 * Inline helpers (T009) — no imports available in the sandbox
 * ------------------------------------------------------------------ */
 ...
 * this function alone sequences phases + stages. No agent() output
 * below is ever consulted to decide *what runs next*.
 * ================================================================== */
...
export default async function run(args) {
  ...
  const settings = await agent(
    ...
    { schema: settingsReadSchema, label: "read-settings", model: FAST, allowedTools: ["Read"] }
  );
  ...
  await phase("collect", async () => {
    ...
    // T013/T014/T015/T017 — per source: open the filtered search, then fetch each posting with pacing.
    await pipeline(
      usable,
      async (src) => {
        let opened;
        ...
        for (let i = 0; i < refs.length; i++) {
          ...
          try {
            ...
            if (res.written) {
              ...
              await appendProvenance(DATA, RUN, {
          ...
          // T015 — pacing between fetches, driven off the loop index (frozen clock; no Date.now()).
          if (i < refs.length - 1 && fetched < FETCH_CAP) {
            await sleep(PACING_MS);
          }
        }
      },
      { concurrency: 1 } // one source at a time keeps pacing meaningful + the user's session off rate limits
    );
  ...
  /* ============================ TRIAGE ============================ */
  // Snapshot of the Raw Record archive after collect — index only; bodies stay on disk (Principle V).
  ...
  await phase("normalize", async () => {
    ...
    const canon = await agent(
      ...
      { schema: dedupGroupSchema, label: "canonicalise-companies", model: FAST, allowedTools: READ_WRITE_TOOLS }
    );
    ...
    await pipeline(
      ...
      async (rec) => {
        const fields = await agent(
          ...
          { schema: jobRecordFieldsSchema, label: `normalize:${slug(rec.id)}`, model: FAST, allowedTools: [] }
        );
 ...
 * ------------------------------------------------------------------ */
async function appendProvenance(dataDir, run, entry) {
  ...
  await agent(
    ...
    {
      schema: { type: "object", additionalProperties: false, required: ["appended"], properties: { appended: { type: "boolean" } } },
      label: "provenance",
      model: FAST,
